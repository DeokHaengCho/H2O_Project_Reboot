...
const CSReview = ({match}) => {
  ...
  useEffect(() => {
    axios
      .get(`http://13.125.133.39:8080/H2O/board/list/getOne/${match.params.boardNo}`)
      .then((res) => {
...
//게시물 수정후 업데이트
  const updateBoard = e => {
    ...
    axios
      .patch(`http://13.125.133.39:8080/H2O/board/modify/${boardNo}`, uploadData)
      .then((res) => {
...
//게시물 삭제
  const deleteBoard = e => {
    e.preventDefault()
    axios
      .delete(`http://13.125.133.39:8080/H2O/board/list/delete/${match.params.boardNo}`)
      .then((res) => {
        console.log(res)
        history.push('/Community/CustomerServiceCenter')
      })
